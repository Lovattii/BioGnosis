...
class BezierView @JvmOverloads constructor(
    ...
    var profundidade_bezier = 0f.dp(context)
        set(value) {
            field = value
            requestLayout()
            invalidate()
    ...
    var largura_bezier = 0f.dp(context)
        set(value) {
            field = value
            requestLayout()
            invalidate()
        }
    var curvinha = 0f.dp(context)
        set(value) {
            field = value
            requestLayout()
            invalidate()
        }
    var CircleSize = 64f.dp(context)
        set(value) {
            field = value
            requestLayout()
            invalidate()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        bezierInnerWidth = (CircleSize*2f)
        if (largura_bezier != -1f)
            bezierInnerWidth = largura_bezier

        bezierInnerHeight = (CircleSize*0.22f)
        if (profundidade_bezier != -1f)
            bezierInnerHeight = profundidade_bezier
    ...
    private fun calculateInner() {
            ...
            // --- AQUI ESTÁ O TRUQUE ---
            // Definimos uma profundidade FIXA para a curva (ex: 72dp)
            // Isso impede que ela estique até o infinito quando você aumenta a barra
            var curveDepth = 0.58f*CircleSize
            if (curvinha != -1f)
                curveDepth = curvinha

            // O fundo da curva será: Ponto de início (bezierInnerHeight) + Profundidade fixa
            val curveBottom = bezierInnerHeight + extra + curveDepth
            // --------------------------
            ...
            // MUDANÇA AQUI: Usamos 'curveBottom' em vez de 'height - extra'
            innerArray[3] = PointF(bezierX - bezierInnerWidth / 4, curveBottom)
            innerArray[4] = PointF(bezierX, curveBottom)
            innerArray[5] = PointF(bezierX + bezierInnerWidth / 4, curveBottom)
            ...
            // MANTENHA AQUI COMO 'height': Isso garante que a barra vá até o final da tela
