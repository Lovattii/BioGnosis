...
class MeowBottomNavigationCell @JvmOverloads constructor(
    ...
    var myCircleSize = 64f.dp(context)
        set(value) {
            field = value
            if (allowDraw) {
                binding.vCircle.layoutParams.width = value.toInt()
                binding.vCircle.layoutParams.height = value.toInt()
                binding.vCircle.requestLayout()

                iconSize = (4f * value) / 5f
            }
        }
    ...
    private var iconSize = (4f*myCircleSize)/5f
        set(value) {
            field = value
            if (allowDraw) {
    ...
    private var progress = 0f
        set(value) {
            ...
            val m = 24.dp(context)
            binding.vCircle.x = (1f - progress) * (if (isFromLeft) -m else m) + ((measuredWidth - myCircleSize) / 2f)

            val circleSelectedY = -15f.dp(context) * (myCircleSize/64f.dp(context))
            val circleUnselectedY = measuredHeight.toFloat() // Esconde lá embaixo quando não selecionado

            binding.vCircle.y = circleUnselectedY + (progress * (circleSelectedY - circleUnselectedY))

            val iconNormalY = (measuredHeight - iconSize)/2f
            val offsetToCenter = (myCircleSize - iconSize) / 2f
            val iconSelectedY = circleSelectedY + offsetToCenter
            binding.fl.y = iconNormalY + (progress * (iconSelectedY - iconNormalY - 10f.dp(context)))        }
    ...
    init {
        ...
        // Isso força a bolinha a ficar do tamanho que você definiu
        binding.vCircle.layoutParams.width = myCircleSize.toInt()
        binding.vCircle.layoutParams.height = myCircleSize.toInt()
        binding.vCircle.requestLayout()
        // -----------------------------
        clipToPadding = false
        clipChildren = false
